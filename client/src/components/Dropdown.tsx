...
const Dropdown: React.FC<DropdownProps> = ({ options, onSelect }) => {
  ...
  return (
    <div className="relative inline-block text-left w-full" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 w-full mt-2 origin-top-right bg-white dark:bg-neutral-700 border border-gray-300 dark:border-neutral-800 rounded-md shadow-lg max-h-40 overflow-y-auto">
          <div className="py-1">
            {options.map((option) => (
              <button
                key={option.value}
